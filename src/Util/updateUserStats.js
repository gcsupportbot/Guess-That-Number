		...
		r.table('user_statistics').get(msg.author.id).run((error, response2) => {
			...
			if (response2) {
				r.table('user_statistics').get(msg.author.id).update({
					...
					extreme_games_played: response.difficulty === 4 ? response2.extreme_games_played + 1 : response2.extreme_games_played,
					...
					extreme_guesses: response.difficulty === 4 ? response2.extreme_guesses + response.score : response2.extreme_guesses,
					...
					extreme_game_time: response.difficulty === 4 ? Number(response2.extreme_game_time) + (Date.now() - Number(response.start_time)) : response2.extreme_game_time,
				...
				r.table('user_statistics').insert({
					...
					extreme_games_played: response.difficulty === 4 ? 1 : 0,
					...
					extreme_guesses: response.difficulty === 4 ? response.score : 0,
					...
					extreme_game_time: response.difficulty === 4 ? Date.now() - Number(response.start_time) : 0,
					coins: coinsAwarded
				}).run((error) => {
					if (error) return reject(error);
					resolve(coinsAwarded);
