...
module.exports = {
	...
	execute: (bot, r, msg, args) => {
		if (args.length > 0) {
			resolveUser(bot, args.length > 0 ? args.join(' ') : msg.author.id).then((user) => {
				if (user.bot) return msg.channel.createMessage(':question: │ Bots are not able to play this game, so they do not have statistics.');
				...
				msg.channel.createMessage(':question: │ I do not know who `' + args.join(' ') + '` is.');
			});
		} else {
			r.table('games').count().run((error, activegames) => {
				if (error) return handleDatabaseError(error, msg);
				snekfetch.get('https://api.github.com/repos/PassTheMayo/Guess-That-Number/commits?per_page=3').then((result) => {
					msg.channel.createMessage({
						embed: {
							...
							fields: [
								{
									name: 'Latest Updates',
									value: result.body.map((c) => '[`' + c.sha.substring(0, 7) + '`](' + c.commit.url + ') ' + c.commit.message).join('\n'),
									inline: false
								},
								{
									name: 'Servers',
